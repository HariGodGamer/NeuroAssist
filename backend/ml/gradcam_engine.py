...
def generate_brain_heatmap_slices(
    ...
    # SimpleITK / PyTorch array order is (Z, Y, X) for array index slice, let's extract cleanly:
    # Axial: constant Z (cz)
...
def _render_heatmap_overlay(brain_slice: np.ndarray, cam_slice: np.ndarray, output_path: str, alpha: float = 0.45):
    ...
    # Blend using alpha. Keep background pixels (where cam is low) mostly structural
    overlay = brain_rgb * (1.0 - alpha * cam_arr[..., None] * 0.8) + heatmap_rgb * (alpha * cam_arr[..., None] * 0.8)
    
    # Blend with direct linear overlay
    overlay_final = brain_rgb * (1.0 - alpha) + heatmap_rgb * alpha
    ...
    # Add clinical panel border
    final_output[0:3, :, :] = [103, 228, 153] # green theme highlight border
    final_output[-3:, :, :] = [103, 228, 153]
    final_output[:, 0:3, :] = [103, 228, 153]
    final_output[:, -3:, :] = [103, 228, 153]
